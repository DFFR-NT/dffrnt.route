	...
	module.exports = function Session(server, app) {
		...
		// Handle Requires ------------------------------------------------------------------------------
			...
			// Config Vars
				...
				const 	Limits 		= redis(config.store);
		...
		// Handle Exposure ------------------------------------------------------------------------------
			...
			THS.Stores 		= { Client: Client, Users: Users, Limits: Limits };
		...
		// Handle Sessions ------------------------------------------------------------------------------
			...
			// This endpoint is hit when the browser deals with Sessions
				...
				if (!!THS.Auth.Flush) CliInit.func = ()=>this.cli.flushdb();
				...
				new ELOGR(Limits, redisPort, 'Limits',  {
					connect: 	{ msg:'initialized', func() {
						this.cli.select(2, this.log({kind:'err'}));
						if (!!THS.Auth.Flush) this.cli.flushdb();
					} 	},
					error: 		{ kind:'err' },
					end: 		{ msg:'Terminated', clr:'grey' },
