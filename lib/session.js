...
module.exports = function (server, app, connection) {
	...
	// Setup Auth/Sessions --------------------------------------------------------------------------
		...
		// User Auth Config
			if (useLDAP) {
				...
				const authSQL = "SELECT * FROM users WHERE email_address = ?";
				passport.use('local-login', new loclauth(authCFG,
					function(req, username, password, next) { // callback with email and password from our form
						...
						Connection.acquire(function (con) {
							...
							con.query(authSQL,[username], function(err, rows) {
								...
								if (!rows.length) info = 'No User with that Email, sorry.';
									...
									console.log('Got Here Too', md5(password));

								// if the user is found but the password is wrong
								// if (md5(password) !== rows[0].password) info = 'Wrong password.';
								// all is well, return successful user

									// console.log('RSLT:',info, rows[0], err);
