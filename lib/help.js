...
// FUNCTIONS
	...
	function Get	(Name, Route) {
		let RGX = new RegExp(`^${(Route||'').replace(/\/$/,'')}$`),
	...
	function Append	(Name, RQ) {
		...
		switch (true) {
			...
			case !!RQ.Params: 	Object.keys(RQ.Params).map((pr, p) => {
