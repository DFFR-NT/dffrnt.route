	...
	function AddSpaces (spaces) {
		...
		async function MrkSpace(spaces) {
			let util = require('util'),
				rdfl = util.promisify(fs.readFile),
				fldr = Publics.Folder.index;
			// ------------------------------------------
			try { 
				if (!!!spaces) throw new Error('No Spaces');
				return await rdfl(fldr, 'utf8'); 
			} catch (err) {
				LG.Error(err, 'EXTERNAL', 'Markup');
				return null;
			}
		}
					...
					// Determine Socket's Intention
						if (!globl) {
							if (accessor) {
								...
								var send = res => socket.compress(true).emit(
			...
			return function AddingSite(req, res) {
				let path  = req.originalUrl,
					...
					HTM   = async (usr,cfg,lid,rnd,res,ret=null) => {
								let cont, stat, rep, ctn, pay, eif = false;
								// -----------------------------------------------
								...
								// -----------------------------------------------
									cont = JSON.stringify([rnd.Build(pay)]);
									stat = await LOKR.set(lid,cont,'EX',300);
				...
				if (ICO(hdrs,url)) {
					...
					LG.Timed(async () => {
						...
						// ------
						rnd = Renders(global,cfg,REST,usr,path,lid);
						res.header(head);
						// ------
						try {
							if (!!rnd.Call) {
					...
					},	req.sessionID, 'EXTERNAL', req.originalUrl, 'magenta');
				};
			};
		}

		// Configure External/Socket Routes
		// Initialize the Custom Namespaces, if available
		MrkSpace(spaces).then(mark => {
			...
			Imm	.OrderedMap(spaces)
				...
				.map((v,k) => {
					let c = v.config,
						...
						gbl = EXPOSES.has(nme);
					// Create Socket Space
					Spaces[k] = AddSpace(nme, acc); LogSpace(nme, dsc);
					...
					switch (true) {
						...
						default: 	let rou = Express.Router(), ste, 
