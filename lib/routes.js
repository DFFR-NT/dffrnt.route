...
// ROUTE HANDLERS
	...
	function ReqSocket (sid, socket, action, path, key, validation, scheme) {
		var key = TLS.Path([action, key]),
			...
			err = ReqHandle.Errs(pnt, REST.MSG.BAD_REQ),
			url = `/${action}/${path}`.toLowerCase().replace(/\/{2,}/g,''),
		...
		socket.on(key, req => {
			...
			validation(req, res, () => { 
				let url = req.originalUrl,
					...
					sch = `${url}/${prm}`;
				// console.log(sch, sch.match(scheme))
						...
						// Handle Good-Guy Requests
							allowd.map(M => {
								let func = M.toLowerCase(),
									schm = new RegExp(
											`${name}\\/${scheme.slice(1,-1)}`
												.replace(/\/{2,}/g,'')
												.replace(/([^\\])\//g,'$1\\/')
												.replace(/([\/(]):\w+(?=[(])/g,'$1')
												.toLowerCase()
											);
								SockRoute(name, pth, key, vld, schm); // Socket Request
