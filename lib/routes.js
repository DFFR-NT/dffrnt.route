...
// ROUTE CONFIGURATION
	...
	function AddRoutes (routes, kind) {
		...
		function AddRoute (route, name, kind) {
			...
			// Error Handles
				Imm.Map(route.Errors).map((paths, kind) => {
	...
	const 	MAKER = {
				Init(api, express, sess, setting) {
					...
					API // Main Router Handles
						...
						.all("/", ReqHandle.Valid(true), (req, res) => {
							// Configure Main Route
							res.status(200)
							   .send(JSN.Valid(REST.Help, {}, {}, 1, 'General Help'));
						})
						.all("/help", ReqHandle.Valid(true), (req, res) => {
							// Configure Main Route
							res.status(200)
							   .send(JSN.Valid(REST.Help, {}, {}, 1, 'General Help'));
						});
