...
// Data Handler
	...
	class DBRequest extends GNRequest {
		...
		/// UTILITIES ///////////////////////////////////////////////////////////////////////
			...
			Opts  	(cls) { return JSN.MapEdit(cls, function (val) { return val.split(';'); }); }
			SQL   	(RQ) { 
				var Q = RQ.Query, AAR = ISS(Q)==='array';
				return (AAR?Q.join("\n").replace("\t",""):Q);
			}
		...
		/// INITIALIZER /////////////////////////////////////////////////////////////////////
			...
			Init  (defaults) {
				...
				THS.Requests.map(function (RQ, R) {
					...
					// Request Handler
					THS[RQ.Name] = function (req, res, next) {
						...
						CONN.acquire(
							function (con) { // Connection Succeeded
								...
								con.query({ 
									...
									try { switch (true) {
											case !!!ret: ret = []; break;
											case TYPE(ret[0],'OkPacket'): 
												LG.IF('STATUS:',ret.slice(0,-1),'\n')
												ret = ret.slice(-1)[0]; break;
										};	itms = THS.Parse(RQ, ret, qry); 
							...
							function (err) { // Connection Failed
...
// EXPORT
	...
	function BINDER (func) {
		var RX = /^function \(\) \{([\S\s]+)\};?$/,
			FN = new Function(
				'SQL','AMP','ORS','PIP','UER','MSG','PRM','Docs','LG','TLS','JSN','Imm',
				func.toString().match(RX)[1]
			);
		return FN(SQL,AMP,ORS,PIP,UER,MSG,PRM,Docs,LG,TLS,JSN,Imm);
	}
