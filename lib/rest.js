...
// REQUIRES
	...
	let Help 		= new Helper({ Params: {
		...
		Points 		= {}, Config = {}, Plugins = {}, _Remote = null,
		CONN 		= new Connection();
	...
	class AURequest extends GNRequest {
		/// CONSTRUCTOR /////////////////////////////////////////////////////////////////////
			...
			constructor(Name, Configs) {
				...
				Help.Create(Name);
				// this.Requests.map((RQ, R) => RQ.Methods.map(
				// 	M => Help.Append(Name, RQ.Routes, RQ.Name, M, RQ[M])
				// )	);
			}
			...
			Session   		(config) {
				...
				return async (req, res, next) => {
					...
					try {
						...
						// console.log('PATH:', { qry: req.query, bdy: req.body, });
				...
				THS.Requests.map(
					...
					function (RT) {
						...
						RT.Methods.map(M=>[M,RT[M]]).filter(M=>!!M[1]).map(MT => {
							...
							// Document ---------------------------------------------------
								Help.Append(THS.Name, RT.Routes, RT.Name, MD, MQ)
							// Define Handler ---------------------------------------------
...
// Data Handler
	...
	class DBRequest extends GNRequest {
		...
		/// UTILITIES   /////////////////////////////////////////////////////////////////////
			 ...
			 * @param {ReqMethod} method The method of this Endpoint
				...
				// ----------------------------------------------------------
					keys.map(function (ky, k) { 
						let prm = ky.toLowerCase(), 
							dfl = THS.Defaults[ky], 
							val = cls[prm]||dfl, 
							fnc = THS.Sanitzers[name][method][ky],
							res;
						try {
							res = fnc(val); 
							cls[prm] = res||dfl; 
						} catch(e) {
							console.log({ name, method, ky, fnc })
							throw e;
						}
					});
				// ----------------------------------------------------------
		...
		/// INITIALIZER /////////////////////////////////////////////////////////////////////
			 ...
			 */
			Init  () {
				var THS  = this, dirty = (v)=>(v), 
					DocH = Imm.Map(Help.Defaults.Headers),
					DocP = Imm.Map(Help.Defaults.Params),
					PWH  = { 'Header': DocH, 'Param': DocP },
					PER  = (f,k,n,e)=>{ e.message=`${f} :: [${k}] ${n} | ${e.message}`; throw e; },
					PFL  = (n)=>(PWH[k].filter((v)=>(!!v.Aliases&&v.Aliases.has(n))));
				// Default Param Handlers -----------------------------------------------------------------------
					...
					function GetParamAliasFactory(kind) { 
						...
						return function GetParamAlias(name) { 
							return doc.filter((v)=>(!!v.Aliases&&v.Aliases.has(name)));
						};
					}
					...
					function FNQueryFactory(RQ) {
						return async function FNQuery(cls, onTimer, onSuccess, onFailure) {
							try { 
								...
								LG.IF(`\n${Q}\n`); eval(F); onTimer(); 
								return onSuccess(...(await FNC(cls())));
							} catch (err) { 
				...
				// Subscribe Requests ---------------------------------------------------------------------------
					THS.Requests.map(
						...
						function (RT) {
							...
							let Parents = THS.Path(!!RT.Routes?RT.Routes:[]),
