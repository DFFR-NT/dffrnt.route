		...
		/// INITIALIZER /////////////////////////////////////////////////////////////////////
			...
			Init  (defaults) {
				...
				THS.Requests.map(function (RQ, R) {
					...
					// Setup Defaults
					Imm.Map(RQ.Params).map(function (v,k,i) {
					...
					// Request Handler
					THS[RQ.Name] = /*preARGS(*/function (req, res, next) {
						...
						CONN.acquire(
							function (con) { // Connection Succeeded
								...
								con.query({ sql: sql, typeCast: THS.Cast(RQ)
								}, function (err, ret) {
									/*end = new Date();*/
...
// EXPORT
	...
	module.exports = {
		...
		DBRequest(name, configs) { Points[name] = new DBRequest(name, configs.Actions).Init(); 	},
		Points: Points,
		Config: Config,
		SQL: 	SQL,
		UER: 	UER,
