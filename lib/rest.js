...
// Auth Handler
	...
	/// UTILITIES ///////////////////////////////////////////////////////////////////////////
		DEFINE(AURequest.prototype, {
			...
			Format:  HIDDEN(function (user) {
				var THS = this, memberOf = THS.Scopes(user).gtcorp, photo = THS.Photo(user), DST = {};
				memberOf.DistributionGroups.map(function (v,i,a) { DST[i] = v; });
				return {
					Account: user.sAMAccountName,
					Profile: Assign( photo, {
						Name: { First: user.givenName, Last: user.sn },
						Business: {
							Title: user.title, Department: user.department,
							Company: user.company, Manager: THS.Boss(user.manager)
						},
						Contact: {
							Email: {
								Address: user.mail, Distribution: DST //memberOf.DistributionGroups
							},
							Phone: {
								Home: { '#': user.telephoneNumber, 'Ext': user.physicalDeliveryOfficeName },
								Mobile: { '#': user.mobile },
							},
							Location: { City: user.l, Region: user.st, Country: user.c },
						},
						Scopes: memberOf.SecurityGroups,
					})
				}
			}),
	...
	/// FUNCTIONS ///////////////////////////////////////////////////////////////////////////
		DEFINE(AURequest.prototype, {
			DeToken:  HIDDEN(function (acct, token, next) {
				var THS = this;
			...
			Tokenize: HIDDEN(function (user, next) {
				var THS = this, acct = user.Account, scopes = user.Profile.Scopes, payload = Assign({
				...
				THS.Token.encode( THS.Cookie.Secret, payload, function (err, token) {
					if (err) { LG.Error(' {....} ', 'Token', TLS.Concat(err.name, err.message)); }
					else { LG.Server(' {....} ', 'Token', acct, 'magenta'); THS.Users.set(acct, token); next(token); }
				});
			}),
			Grant: 	  HIDDEN(function (user, withToken, res, next) {
					...
					switch (true) {
						...
						case !!!token: 	THS.Tokenize(user, done); break;;
							  default: 	THS.DeToken(acct, token, function (scp) {
							  				var scopes = user.Profile.Scopes;
			...
			Profile:  HIDDEN(function (acct, withToken, res, next) {
				var THS = this, erQ = { path: '/error' }, usr = {};
				THS.LDAP.Search(acct, function (err, data) {
					switch (true) {
						case  !!err:  THS.ER(res, MSG.ERROR,  err, acct, erQ); break;;
						case !!!data: THS.ER(res, MSG.EXISTS, UER, acct, erQ); break;;
						default:
							data.on('searchEntry', function (entry) {
								var obj = entry.object, raw = entry.raw, photo = raw.thumbnailPhoto;
								if (photo) { obj.thumbnailPhoto = photo.toString('base64'); };
								usr = THS.Format(obj);
							})
							.on('error', function (err) { THS.ER(res, MSG.ERROR, err, acct, erQ); })
							.on('end', 	 function () {
								if (!!usr.Profile) THS.Grant(usr, withToken, res, next);
								else THS.ER(res, MSG.EXISTS, UER, acct, { path: '/users' });
							});
					}
				});
			}),
			Renew: 	  HIDDEN(function (req) {
				var THS = this, MX = THS.Cookie.Age, EX = req.session.cookie.maxAge;
				if ( EX / MX <= 0.20 ) { req.session.touch(); req.session.save(); }
			}),
			Session:  HIDDEN(function (config) {
				var THS = this; return function (req, res, next) {
					var sess = req.session, sid = req.sessionID;
					// --
					try {
						var Pick = function (which, obj) {
...
// EXPORT
	...
	module.exports = {
		Init: 		function (session) { Connection.init(); Session = session; },
